...
def complete_quest(id):
  ...
  if id not in state["completed"] and id <= len(quests):
    _last_task_completed_at = badge.ticks
...
def update():
  ...
  if badge.pressed() and _last_task_completed_at:
    _last_task_completed_at = None
  ...
  if _last_task_completed_at:
    ...
    zoom_speed = 250
    if badge.ticks - _last_task_completed_at < zoom_speed:
      # for first 250ms of well done screen animate it zooming in
      alpha = ((badge.ticks - _last_task_completed_at) / zoom_speed)
      zoom = ((badge.ticks - _last_task_completed_at) / zoom_speed) * 10
      width *= (zoom / 10)
      height *= (zoom / 10)
      splash.alpha = int(alpha * 255)
      _scale_blit(splash, 80 - width / 2, 60 - height / 2, width, height)
    else:
      splash.alpha = 255
      ...
      screen.font = large_font
      screen.text(label, int(80 - (lw / 2)), 2)
      screen.font = small_font
      screen.text(message, int(80 - (mw / 2)), 19)
