...
# v2.0.2 port: badgeware names are builtins; run/screen/image/color/shape/vec2/badge global.
import badgeware  # noqa: F401
screen.antialias = OFF  # 1px pixel drawing tool — AA would blur single pixels
import ui


canvas = image(ui.canvas_area[2], ui.canvas_area[3])  # v2.0.2: image(w, h) blank canvas
...
# v2.0.2 launch() contract: apps run at import (no __main__ guard).
run(update)
